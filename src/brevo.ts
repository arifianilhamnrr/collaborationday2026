...
export function confirmedPaymentEmailContent(appOrigin: string, recipient: { participantName: string; receiptNumber: string; groupName: string | null; groupWhatsappUrl: string | null }): string {
  const groupAccess = recipient.groupName
    ? `<p>Kamu tergabung dalam <b>Kelompok ${escapeHtml(recipient.groupName)}</b>.</p>${recipient.groupWhatsappUrl ? `<p><a href="${escapeHtml(recipient.groupWhatsappUrl)}">Gabung grup WhatsApp</a></p>` : '<p>Tautan grup WhatsApp sedang disiapkan pendamping dan akan muncul di dashboard setelah tersedia.</p>'}`
    : '';
