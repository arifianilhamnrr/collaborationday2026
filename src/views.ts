...
export function adminParticipantsPage(
  ...
  return dashboardLayout(
    "Peserta",
    `${message ? `<div class="notice">${escapeHtml(message)}</div>` : ""}<div class="page-head"><div><h2>Data peserta.</h2><p>Filter status pembayaran, kelompok, dan urutan peserta tanpa memuat ulang halaman.</p></div><a class="app-button" href="/dashboard/participants/export.csv">Export peserta confirmed</a></div>${table}<script src="/client-tables.js?v=20260827-filters" defer></script>`,
    user,
    "participants",
  );
}
