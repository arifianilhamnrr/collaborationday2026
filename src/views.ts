...
.participant-proof-links{display:flex;align-items:center;gap:7px;flex-wrap:wrap}.participant-proof-links a{display:inline-flex;align-items:center;min-height:34px;padding:7px 10px;border:1px solid var(--app-line);background:#fff;color:var(--app-ink);font-size:11px;font-weight:800;text-decoration:none}.participant-proof-links a:hover{border-color:var(--app-accent);color:var(--app-accent)}.proof-preview-actions{display:flex;align-items:center;gap:10px}.proof-preview-actions a{font-size:12px;font-weight:800}.proof-preview-actions button{min-width:76px}
.table-tools{display:flex;align-items:end;justify-content:space-between;gap:14px;margin:14px 0;flex-wrap:wrap}.table-search{flex:1;min-width:220px;max-width:520px}.table-search span,.table-page-size span,.table-filter span,.table-sort span{display:block;margin-bottom:5px;color:var(--app-muted);font-size:11px;font-weight:800}.table-page-size select{min-width:82px}.participant-table-tools{display:grid;grid-template-columns:minmax(220px,1.4fr) repeat(4,minmax(130px,.72fr));align-items:end}.participant-table-tools .table-search{min-width:0;max-width:none}.table-pagination{display:flex;align-items:center;justify-content:space-between;gap:16px;padding-top:14px}.table-pagination p{margin:0;color:var(--app-muted);font-size:12px}.table-page-actions{display:flex;align-items:center;gap:8px}.table-page-actions button{width:auto;min-width:96px;padding:9px 12px}.table-page-actions span{min-width:92px;text-align:center;font-weight:800}.table-no-results td{text-align:center;color:var(--app-muted);padding:28px 16px}
@media(max-width:960px){.sidebar-toggle,.sidebar-close{display:inline-flex}.sidebar-overlay{display:block}}
@media(max-width:900px){.participant-table-tools{grid-template-columns:repeat(2,minmax(0,1fr))}.participant-table-tools .table-search{grid-column:1/-1}}
@media(max-width:640px){.session-actions{justify-content:flex-start}.session-action{flex:1}.app-dialog-actions{display:grid}.app-dialog-actions button{width:100%}.profile-meta,.balance{grid-template-columns:1fr}.app-topbar>.app-button{display:none}.social-account{grid-template-columns:44px minmax(0,1fr);gap:13px;padding:14px}.social-platform{width:44px;height:44px}.social-cta{grid-column:2;justify-self:start}.social-copy span{white-space:normal;overflow-wrap:anywhere}.registration-summary-head{padding:22px 18px}.registration-facts{grid-template-columns:1fr}.registration-fact{padding:17px 18px;border-right:0;border-bottom:1px solid var(--app-line)}.registration-fact:last-child{border-bottom:0}.registration-action{display:grid;padding:18px}.registration-action button{width:100%;min-width:0}.payment-overview-facts,.bank-details{grid-template-columns:1fr}.payment-overview-facts>div+div{border-left:0;border-top:1px solid var(--app-line)}.payment-option-head{align-items:flex-start;padding:16px}.payment-option-body,.payment-action,.payment-selector{padding:16px}.table-tools{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:end}.participant-table-tools{grid-template-columns:1fr}.participant-table-tools .table-search{grid-column:auto}.table-search{max-width:none}.table-pagination{display:grid}.table-page-actions{justify-content:space-between}.table-page-actions button{min-width:0;flex:1}}
`;
...
export function participantDashboard(
  ...
  if (!user.email_verified_at) {
    ...
    const paymentFlow = methods.length ? `<div class="payment-selector"><label>Pilih metode pembayaran<select id="participant-payment-method" required><option value="">Pilih salah satu metode</option>${paymentChoices}</select></label></div><div class="participant-payment-options" aria-live="polite"><div class="payment-empty" id="participant-payment-empty">Pilih metode pembayaran untuk melihat detail dan melanjutkan.</div>${paymentOptions}</div><script src="/participant-payment.js?v=20260826-stable-upload" defer></script>` : '<div class="notice">Belum ada metode pembayaran yang aktif.</div>';
    ...
    const cashReviewContact = registration.status === "payment_review" && registration.pending_payment_type === "cash" ? participantCashContact(registration, profile.full_name) : "";
    content = `<span class="status">${escapeHtml(labels[String(registration.status)] ?? registration.status)}</span><h1>Halo, ${escapeHtml(profile.full_name)}.</h1>${eventOverview}${confirmed ? `<div class="notice"><b>Participant pass aktif.</b><br>Reference QR: <span class="mono">${escapeHtml(registration.ticket_reference)}</span></div>${registration.receipt_id ? `<a class="button" href="/dashboard/receipts/${escapeHtml(registration.receipt_id)}">Unduh kuitansi elektronik</a>` : "<p>Kuitansi elektronik sedang diproses.</p>"}` : `${registration.rejection_reason ? `<div class="notice error"><b>Pembayaran ditolak:</b> ${escapeHtml(registration.rejection_reason)}</div>` : ""}${registration.status === "payment_review" ? `<div class="notice"><b>Pembayaran sedang diproses.</b><br>${registration.pending_payment_type === "cash" ? "Hubungi pendamping untuk mengatur penyerahan dan pencatatan uang tunai." : "Kamu tidak perlu mengirim bukti kembali selama proses review."}</div>${cashReviewContact}` : paymentFlow}`}`;
    if (registration.group_name) content += `<div class="notice"><b>Kelompok ${escapeHtml(registration.group_name)}</b>${confirmed ? registration.group_whatsapp_url ? `<br><a href="${escapeHtml(registration.group_whatsapp_url)}" target="_blank" rel="noreferrer">Gabung grup WhatsApp</a>` : "<br>Tautan grup WhatsApp sedang disiapkan pendamping." : "<br>Tautan grup WhatsApp tersedia setelah pembayaran terverifikasi."}</div>`;
  ...
  content = content
    .replace("Langkah 1 dari 6", "Langkah 1 dari 5")
    .replace("Langkah 2 dari 6", "Langkah 2 dari 5")
...
function participantPaymentOption(method: PaymentMethod, user: SessionUser, registration: Record<string, unknown>, participantName: string): string {
  ...
  const cashContact = participantCashContact(registration, participantName);
  ...
  const action = method.type === "cash"
    ...
    : `<form class="stack payment-action" method="post" action="/dashboard/payment-proof" enctype="multipart/form-data" data-stable-upload>${csrf}<input type="hidden" name="payment_method_id" value="${method.id}"><label>Bukti pembayaran<input name="proof" type="file" accept="image/jpeg,image/png,application/pdf" required data-stable-file></label><small>Format JPG, PNG, atau PDF; maksimal 5 MB.</small><small data-upload-status aria-live="polite"></small><button type="submit">Kirim bukti pembayaran</button></form>`;
...
function participantCashContact(registration: Record<string, unknown>, participantName: string): string {
  const pendampingPhone = String(registration.pendamping_phone || "").replace(/\D/g, "");
  const pendampingName = String(registration.pendamping_name || "pendamping kelompok");
  const message = `Halo ${pendampingName}, saya ${participantName} (${String(registration.public_id || "peserta")}) dari Kelompok ${String(registration.group_name || "-")}. Saya ingin melakukan pembayaran tunai Collaboration Day.`;
  return pendampingPhone ? `<p><a class="button secondary" href="https://wa.me/${escapeHtml(pendampingPhone)}?text=${encodeURIComponent(message)}" target="_blank" rel="noreferrer">Hubungi pendamping via WhatsApp</a></p>` : '<div class="notice">Nomor WhatsApp pendamping belum tersedia. Hubungi panitia untuk bantuan pembayaran tunai.</div>';
}
...
export function adminTeamPage(
  ...
  const staffRows = staff.map((member) => `<tr data-table-row data-search="${escapeHtml(`${member.full_name || ""} ${member.email || ""} ${member.role || ""} ${member.phone_e164 || ""}`)}"><td><b>${escapeHtml(member.full_name || "Belum melengkapi profil")}</b><br><small>${escapeHtml(member.email)}</small></td><td><span class="status">${escapeHtml(member.role)}</span></td><td>${member.phone_e164 ? escapeHtml(member.phone_e164) : "—"}</td><td>${member.phone_e164 ? "Tersimpan" : "Belum diisi"}</td></tr>`).join("") || '<tr><td colspan="4">Belum ada staf.</td></tr>';
  const groupRows = groups.map((group) => `<tr data-table-row data-search="${escapeHtml(`${group.name || ""} ${group.pendamping_name || ""} ${group.pendamping_email || ""} ${group.member_count || 0}`)}"><td><b>${escapeHtml(group.name)}</b></td><td>${pendamping.length ? `<form class="stack" method="post" action="/dashboard/team/groups/${escapeHtml(group.id)}/pendamping">${csrf}<select name="pendamping_user_id" required><option value="" ${group.pendamping_user_id ? "" : "selected"} disabled>Pilih pendamping</option>${pendamping.map((member) => `<option value="${escapeHtml(member.user_id)}" ${Number(member.user_id) === Number(group.pendamping_user_id) ? "selected" : ""}>${escapeHtml(member.full_name || member.email)}</option>`).join("")}</select><button class="secondary" type="submit">${group.pendamping_user_id ? "Ganti" : "Tetapkan"} pendamping</button></form>` : '<span class="status">Belum ada pendamping</span>'}</td><td>${escapeHtml(group.member_count || 0)} peserta</td><td>${group.whatsapp_invite_url ? `<a href="${escapeHtml(group.whatsapp_invite_url)}" target="_blank" rel="noreferrer">Buka grup</a>` : "Belum diisi"}</td></tr>`).join("") || '<tr><td colspan="4">Belum ada kelompok.</td></tr>';
  const participantRows = participants.map((participant) => `<tr data-table-row data-search="${escapeHtml(`${participant.full_name || ""} ${participant.email || ""} ${participant.group_name || "Belum dibagi"}`)}"><td><b>${escapeHtml(participant.full_name)}</b><br><small>${escapeHtml(participant.email)}</small></td><td>${escapeHtml(participant.group_name || "Belum dibagi")}</td></tr>`).join("") || '<tr><td colspan="2">Belum ada peserta.</td></tr>';
  const staffTable = searchableTable("staff", "staf", "Nama, email, role, atau WhatsApp", "<tr><th>Nama</th><th>Role</th><th>WhatsApp</th><th>Status nomor</th></tr>", staffRows, 4);
  const groupTable = searchableTable("groups", "kelompok", "Nama kelompok atau pendamping", "<tr><th>Kelompok</th><th>Pendamping</th><th>Anggota</th><th>WhatsApp</th></tr>", groupRows, 4);
  const participantTable = searchableTable("participants", "peserta", "Nama, email, atau kelompok", "<tr><th>Peserta</th><th>Kelompok</th></tr>", participantRows, 2);
  const page = dashboardLayout(
    "Tim & Kelompok",
  ...
  return page
    .replace(/<section class="app-card"><h3>Staf operasional<\/h3>.*?<\/section>/s, `<section class="app-card"><h3>Staf operasional</h3>${staffTable}</section>`)
    .replace(/<section class="app-card half"><h3>Kelompok<\/h3>.*?<\/section>/s, `<section class="app-card half"><h3>Kelompok</h3>${groupTable}</section>`)
    .replace(/<section class="app-card half"><h3>Pembagian peserta<\/h3>.*?<\/section>/s, `<section class="app-card half"><h3>Pembagian peserta</h3>${participantTable}</section>`)
    .replace('</body>', '<script src="/client-tables.js?v=20260826" defer></script></body>');
}

function searchableTable(id: string, label: string, placeholder: string, header: string, rows: string, columnCount: number): string {
  return `<div data-client-table="${id}" data-page-size="10"><div class="table-tools"><label class="table-search"><span>Cari ${label}</span><input type="search" data-table-search placeholder="${escapeHtml(placeholder)}" autocomplete="off"></label><label class="table-page-size"><span>Baris</span><select data-table-page-size><option value="10">10</option><option value="25">25</option><option value="50">50</option></select></label></div><div class="app-table"><table><thead>${header}</thead><tbody>${rows}<tr class="table-no-results" data-table-empty hidden><td colspan="${columnCount}">Tidak ada ${label} yang cocok.</td></tr></tbody></table></div><div class="table-pagination"><p data-table-summary aria-live="polite"></p><div class="table-page-actions"><button class="secondary" type="button" data-table-prev>Sebelumnya</button><span data-table-page aria-live="polite"></span><button class="secondary" type="button" data-table-next>Berikutnya</button></div></div></div>`;
}
...
export function adminPaymentsPage(
  ...
  const paymentRows =
    payments
      .map((payment) => {
        const receipt = payment.receipt_id ? `<div class="review-receipt"><a href="/dashboard/receipts/${escapeHtml(payment.receipt_id)}">PDF</a><span class="delivery-chip">Email ${escapeHtml(payment.email_status)}</span><span class="delivery-chip">WA dinonaktifkan</span>${payment.email_status !== "sent" ? `<form method="post" action="/dashboard/receipts/${escapeHtml(payment.receipt_id)}/retry">${csrf}<button class="secondary" type="submit">Kirim ulang email</button></form>` : ""}</div>` : "—";
...
export function adminParticipantsPage(
  ...
  const participantRows =
    participants
      .map(
        (row) => {
          const paymentStatus = participantPaymentStatus(row);
          const paymentLabel = participantPaymentStatusLabel(paymentStatus);
          const group = String(row.group_name || "Belum dibagi");
          const name = String(row.full_name || "");
          const statusOrder = { confirmed: "1", review: "2", rejected: "3", unpaid: "4", unregistered: "5" }[paymentStatus];
          return `<tr data-table-row data-search="${escapeHtml(`${name} ${row.email || ""} ${row.phone || ""} ${group} ${paymentLabel}`)}" data-payment-status="${paymentStatus}" data-group="${escapeHtml(group)}" data-name="${escapeHtml(name)}" data-created="${escapeHtml(row.created_at || "")}" data-status-order="${statusOrder}"><td><b>${escapeHtml(name)}</b><br>${escapeHtml(row.email)}</td><td>${row.gender === "male" ? "Laki-laki" : row.gender === "female" ? "Perempuan" : "Belum diisi"}<br><small>${escapeHtml(group)}</small></td><td>${escapeHtml(row.phone)}<br><span class="status">Tersimpan</span></td><td>Follow: ${row.social_proof_id ? `<span class="status">${escapeHtml(row.social_proof_status || "pending")}</span>` : "—"}<br>Kelulusan: ${row.admission_proof_id ? "✓" : "—"}</td><td><span class="status">${escapeHtml(paymentLabel)}</span>${row.payment_type ? `<br><small>${escapeHtml(paymentTypeLabel(String(row.payment_type)))}</small>` : ""}</td><td>${participantProofLinks(row, name)}</td></tr>`;
        },
      )
      .join("") || '<tr><td colspan="6">Belum ada peserta.</td></tr>';
  const table = `<div data-client-table="participants" data-page-size="25"><div class="table-tools participant-table-tools"><label class="table-search"><span>Cari peserta</span><input type="search" data-table-search placeholder="Nama, email, WhatsApp, atau kelompok" autocomplete="off"></label><label class="table-filter"><span>Status pembayaran</span><select data-table-filter data-field="paymentStatus"><option value="">Semua status</option><option value="confirmed">Terkonfirmasi</option><option value="review">Sedang diverifikasi</option><option value="rejected">Ditolak</option><option value="unpaid">Belum bayar</option><option value="unregistered">Belum daftar</option></select></label><label class="table-filter"><span>Kelompok</span><select data-table-filter data-field="group"><option value="">Semua kelompok</option>${[...new Set(participants.map((row) => String(row.group_name || "Belum dibagi")))].sort((a, b) => a.localeCompare(b, "id-ID")).map((group) => `<option value="${escapeHtml(group)}">${escapeHtml(group)}</option>`).join("")}</select></label><label class="table-sort"><span>Urutkan</span><select data-table-sort><option value="created-desc">Terbaru</option><option value="created-asc">Terlama</option><option value="name-asc">Nama A–Z</option><option value="name-desc">Nama Z–A</option><option value="group-asc">Kelompok A–Z</option><option value="statusOrder-asc">Status pembayaran</option></select></label><label class="table-page-size"><span>Baris</span><select data-table-page-size><option value="10">10</option><option value="25" selected>25</option><option value="50">50</option><option value="100">100</option></select></label></div><div class="app-table"><table><thead><tr><th>Peserta</th><th>Gender & kelompok</th><th>WhatsApp</th><th>Syarat</th><th>Pembayaran</th><th>Dokumen</th></tr></thead><tbody>${participantRows}<tr class="table-no-results" data-table-empty hidden><td colspan="6">Tidak ada peserta yang cocok dengan filter.</td></tr></tbody></table></div><div class="table-pagination"><p data-table-summary aria-live="polite"></p><div class="table-page-actions"><button class="secondary" type="button" data-table-prev>Sebelumnya</button><span data-table-page aria-live="polite"></span><button class="secondary" type="button" data-table-next>Berikutnya</button></div></div></div>`;
  return dashboardLayout(
    "Peserta",
    `${message ? `<div class="notice">${escapeHtml(message)}</div>` : ""}<div class="page-head"><div><h2>Data peserta.</h2><p>Filter status pembayaran, kelompok, dan urutan peserta tanpa memuat ulang halaman.</p></div><a class="app-button" href="/dashboard/participants/export.csv">Export peserta confirmed</a></div>${table}<dialog class="app-dialog proof-preview-dialog" id="participant-proof-dialog"><div class="app-dialog-body"><div class="proof-preview-head"><h2 id="participant-proof-title">Preview dokumen</h2><div class="proof-preview-actions"><a id="participant-proof-open" href="#" target="_blank" rel="noreferrer">Buka tab baru</a><button class="secondary" type="button" data-proof-close>Tutup</button></div></div><iframe class="proof-preview-frame" id="participant-proof-frame" title="Preview dokumen peserta"></iframe></div></dialog><script src="/client-tables.js?v=20260827-filters" defer></script><script src="/participant-proof-preview.js?v=20260827" defer></script>`,
    user,
    "participants",
  );
}

function participantProofLinks(row: Record<string, unknown>, participantName: string): string {
  const links: string[] = [];
  if (row.admission_proof_id) {
    const href = `/dashboard/admission-proofs/${escapeHtml(row.admission_proof_id)}`;
    links.push(`<a href="${href}" target="_blank" rel="noreferrer" data-participant-proof data-proof-url="${href}?preview=1" data-proof-title="Bukti kelulusan · ${escapeHtml(participantName)}">Kelulusan</a>`);
  }
  if (row.social_proof_id) {
    const id = escapeHtml(row.social_proof_id);
    const social = [
      ["collaboration-day-instagram", "CD Instagram"],
      ["hmps-instagram", "HMPS Instagram"],
      ["hmps-tiktok", "HMPS TikTok"],
    ];
    social.forEach(([kind, label]) => {
      const href = `/dashboard/social-proofs/${id}/${kind}`;
      links.push(`<a href="${href}" target="_blank" rel="noreferrer" data-participant-proof data-proof-url="${href}?preview=1" data-proof-title="${label} · ${escapeHtml(participantName)}">${label}</a>`);
    });
  }
  return links.length ? `<div class="participant-proof-links">${links.join("")}</div>` : "—";
}

function participantPaymentStatus(row: Record<string, unknown>): "confirmed" | "review" | "rejected" | "unpaid" | "unregistered" {
  if (row.registration_status === "confirmed") return "confirmed";
  if (row.registration_status === "payment_review") return "review";
  if (row.payment_submission_status === "rejected") return "rejected";
  if (row.registration_status === "pending_payment") return "unpaid";
  return "unregistered";
}

function participantPaymentStatusLabel(status: ReturnType<typeof participantPaymentStatus>): string {
  return { confirmed: "Terkonfirmasi", review: "Sedang diverifikasi", rejected: "Ditolak", unpaid: "Belum bayar", unregistered: "Belum daftar" }[status];
}

function paymentTypeLabel(type: string): string {
  return { cash: "Tunai", static_qris: "QRIS", bank_transfer: "Transfer bank" }[type] || type;
}
