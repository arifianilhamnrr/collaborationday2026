...
export function participantDashboard(
  ...
  if (profile?.phone && profile.group_name && !registration) content = `<div class="notice"><b>Kelompok ${escapeHtml(profile.group_name)}</b>${profile.group_whatsapp_url ? `<br><a href="${escapeHtml(profile.group_whatsapp_url)}" target="_blank" rel="noreferrer">Gabung grup WhatsApp</a>` : "<br>Tautan grup WhatsApp sedang disiapkan pendamping."}</div>${content}`;
